# Author: Al Ahzmir D. de Gracia
# Date Compiled: May 08, 2023
...
def Database():
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS TblIT1R7 (Student_id INTEGER NOT NULL  PRIMARY KEY AUTOINCREMENT, firstname TEXT, lastname TEXT, gender TEXT, age TEXT, address TEXT, contact TEXT)")
    cursor.execute("SELECT * FROM TblIT1R7 ORDER BY lastname ASC")
...
def SubmitData():
    if FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or AGE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "":
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO TblIT1R7 (firstname, lastname, gender, age, address, contact) VALUES(?, ?, ?, ?, ?, ?)", (
...
def UpdateData():
    if GENDER.get() == "":
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE TblIT1R7 SET firstname = ?, lastname = ?, gender =?, age = ?,  address = ?, contact = ? WHERE Student_id = ?",
            (str(FIRSTNAME.get()), str(LASTNAME.get()), str(GENDER.get()), str(AGE.get()), str(ADDRESS.get()),
             str(CONTACT.get()), int(Student_id)))
        conn.commit()
        cursor.execute("SELECT * FROM TblIT1R7 ORDER BY lastname ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def CloseWindow():
    result = tkMessageBox.askquestion('', 'Are you sure you want exit?', icon="warning")
    root.destroy()
...
def DeleteData():
    if not tree.selection():
        ...
        if result == 'yes':
            ...
            conn = sqlite3.connect("contact.db")
            cursor = conn.cursor()
            cursor.execute("DELETE FROM TblIT1R7 WHERE Student_id = %d" % selecteditem[0])
            conn.commit()
            cursor.close()
            conn.close()
...
# ============================FRAMES======================================
Top = Frame(root, width=500, bd=2, relief=SOLID)
Top.pack(side=TOP)
Mid = Frame(root, width=500, bg="#e3e18d")
Mid.pack(side=TOP)
MidLeft = Frame(Mid, width=100)
MidLeft.pack(side=LEFT, pady=15)
MidLeftPadding = Frame(Mid, width=370, bg="#e3e18d")
MidLeftPadding.pack(side=LEFT)
MidRight = Frame(Mid, width=100)
MidRight.pack(side=RIGHT, pady=15)

TableMargin = Frame(root, width=500)
TableMargin.pack(side=TOP)

# ============================LABELS======================================
lbl_title = Label(Top, text="Contact Management System", font=('times', 19), bg="#f6ec7a", width=500)
lbl_title.pack(fill=X)

# ============================ENTRY=======================================

# ============================BUTTONS=====================================
btn_add = Button(MidLeft, text="+ ADD NEW", font=('times', 11), bg="#66ff66", command=AddNewWindow)
btn_add.pack()

btn_add = Button(MidRight, text="CLOSE", font=('times'), bg="#FFA500", command=CloseWindow)
btn_add.pack()

btn_delete = Button(MidRight, text="DELETE", font=('times', 11), bg="red", command=DeleteData)
btn_delete.pack(side=RIGHT)

# ============================TABLES======================================
scrollbarx = Scrollbar(TableMargin, orient=HORIZONTAL)
scrollbary = Scrollbar(TableMargin, orient=VERTICAL)
tree = ttk.Treeview(TableMargin, columns=("StudentID", "Firstname", "Lastname", "Gender", "Age", "Address", "Contact"),
